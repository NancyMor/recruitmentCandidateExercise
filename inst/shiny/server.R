...
server <- function(input, output, session) {
  ...
  output$textWithHTML <- renderUI({
    rawText <- readLines("data/description.txt")
  ...
  output$spend <- renderHighchart({
    ...
    descriptive_plot <- highchart() %>%
      hc_add_series(data = search_data(),
                    "line",
                    hcaes(
                      x = dmy(`Date (Week)`),
                      y = `Media Spend (USD)`,
  ...
  output$descriptive_data <- renderTable({
    descriptive <-
      search_data() %>% .[, Year := year(dmy(`Date (Week)`))] %>% .[, .(Year, `Media Spend (USD)`, `Media Campaign`, `Search Volume`)] %>%
      .[, .(
        `Spend(USD)` = sum(`Media Spend (USD)`),
        `Search Volume` = sum(`Search Volume`)
      ), by = .(Year, `Media Campaign`)]
  ...
  output$correlation <- renderUI({
    cor_base <-
      dcast.data.table(
        search_data(),
        . ~ "Media Campaign",
        value.var = "Media Spend (USD)",
        fun.aggregate = sum,
        fill = NA
      )
    camp1 <-
      search_data() %>% .[`Media Campaign` == 1, ] %>% .[, .(Campign1 = `Media Spend (USD)`, `Search Volume`)] %>% cor() %>% .[1, 2] %>% round(2)
    camp2 <-
      search_data() %>% .[`Media Campaign` == 2, ] %>% .[, .(Campign2 = `Media Spend (USD)`, `Search Volume`)] %>% cor() %>% .[1, 2] %>% round(2)
    camp3 <-
      search_data() %>% .[`Media Campaign` == 3, ] %>% .[, .(Campign3 = `Media Spend (USD)`, `Search Volume`)] %>% cor() %>% .[1, 2] %>% round(2)
  ...
  output$estimated <- renderHighchart({
    ...
    descriptive_plot <- highchart() %>%
      ...
      hc_add_series(data = linear_model(),
                    "line",
                    name = "Search Volume",
                    hcaes(x = dmy(`Date (Week)`), y = `Search Volume`)) %>%
      hc_add_series(
        data = linear_model(),
        "line",
        name = "Estimated Volume",
        hcaes(x = dmy(`Date (Week)`), y = `Estimated Search Volume`)
      ) %>%
      hc_yAxis(title = list(text = "Media Spend (USD)")) %>%
      hc_xAxis(type = 'datetime')
  ...
  output$linear1 <- renderPlotly({
    ...
    data_campaign1 <-
      linear_model %>% as.data.table() %>% .[`Media Campaign`  == 1, ]
    linear_plot <- plot_ly(data_campaign1)
    linear_plot <-
      linear_plot %>% add_markers(x = ~  `Media Spend (USD)` ,
                                  y = ~ `Search Volume`)
    linear_plot <-
      linear_plot %>% add_lines(
        x = ~  `Media Spend (USD)`,
        y = ~ `Estimated Search Volume`,
        line = list(color = '#07A4B5'),
        name = "Loess Smoother",
        showlegend = FALSE
      ) %>%
      layout(
        title = 'Media Campaign 1',
        xaxis = list(title = ""),
        yaxis = list(title = "")
      )
  ...
  output$linear2 <- renderPlotly({
    linear_model <- linear_model()
    data_campaign2 <-
      linear_model %>% as.data.table() %>% .[`Media Campaign`  == 2, ]
  ...
  output$linear3 <- renderPlotly({
    linear_model <- linear_model()
    data_campaign3 <-
      linear_model %>% as.data.table() %>% .[`Media Campaign`  == 3, ]
  ...
  output$total_accuracy <- renderUI({
    ...
    total <-
      linear_model %>% .[, r_square(`Search Volume`, `Estimated Search Volume`)]
  ...
  output$accuracy <- renderTable({
    ...
    #Getting result for total and each campaign
    camp1 <-
      linear_model[`Media Campaign`  == 1,] %>% .[, r_square(`Search Volume`, `Estimated Search Volume`)]
    camp2 <-
      linear_model[`Media Campaign`  == 2,] %>% .[, r_square(`Search Volume`, `Estimated Search Volume`)]
    camp3 <-
      linear_model[`Media Campaign`  == 3,] %>% .[, r_square(`Search Volume`, `Estimated Search Volume`)]
    total <-
      linear_model %>% .[, r_square(`Search Volume`, `Estimated Search Volume`)]

    #R squared result
    r_square_results <-
      data.table("R2" = c(camp1, camp2, camp3),
                 "Media Campaign" = c(1, 2, 3))

    #Paremeters Results
    parameters_table <-
      linear_model[, .(`Media Campaign`, `Beta`, `Intercept`)] %>% unique()

    #Merging table
    table_results <-
      merge(r_square_results, parameters_table, by = "Media Campaign")
  ...
  output$roi <- renderTable({
    ...
    descriptive <-
      ...
      .[, .(
        `Spend(USD)` = sum(`Media Spend (USD)`),
        `Search Volume` = sum(`Search Volume`)
      ), by = .(`Media Campaign`)] %>% .[, ROI := `Search Volume` / `Spend(USD)`]
